...
const EditInterviews: React.FC = () => {
  ...
  const handleSaveContent = useCallback(async () => {
    try {
      ...
      if (title === '') {
        ...
        const response = await api.put('/interviews/update', data);
  ...
  }, [content1, content2, interviews_id, title, updateToken, user.id, video]);
  ...
  }, [imageTop, interviews, updateToken]);
  ...
  useEffect(() => {
    document.title = 'Editar entrevista';
  }, []);

  useEffect(() => {
    async function loadInterviews() {
      try {
        const response = await api.get(`/interviews/show/${interviews_id}`);

        if (response.status === 200) {
          const interviewsData = response.data as IInterviews;

          setInterviews(interviewsData);

          setTitle(interviewsData.title);
          setVideo(interviewsData.video);
          setContent1(interviewsData.content_1);
          setContent2(interviewsData.content_2);
          setPreviewImageTop(interviewsData.image_1_url);
          setPreviewImageMiddle(interviewsData.image_2_url);
        }
      } catch (err) {
  ...
  return (
    <Container>
      ...
      <Content>
        <HeaderContent title="Editar entrevista" />
